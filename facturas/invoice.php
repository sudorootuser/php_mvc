...
/*----------------- Instancia controlador prestamo -----------------*/
...
if ($datos_prestamo->rowCount() == 1) {
	...
	$datos_usuario = $ins_usuario->datos_usuario_controlador("Unico", $ins_usuario->encrypt_decrypt("encrypt", $datos_prestamo['usuario_id']));
	$datos_usuario = $datos_usuario->fetch();

	/*----------------- Instancia controlador cliente -----------------*/
	require_once "../controladores/clienteControlador.php";
	$ins_cliente = new clienteControlador();

	$datos_cliente = $ins_cliente->datos_cliente_controlador("Unico", $ins_cliente->encrypt_decrypt("encrypt", $datos_prestamo['cliente_id']));
	...
	$datos_detalle = $ins_prestamo->datos_prestamo_controlador("Detalle", $ins_prestamo->encrypt_decrypt("encrypt", $datos_prestamo['prestamo_codigo']));
	...
	foreach ($datos_detalle as $items) {
		...
		$pdf->Cell(51, 10, utf8_decode($items['detalle_tiempo'] . " " . $items['detalle_formato'] . " (" . MODENA . $items['detalle_costo_tiempo'] . " c/u)"), 'L', 0, 'C');
		$pdf->Cell(25, 10, utf8_decode(MODENA . $subtotal), 'LR', 0, 'C');
	...
	$pdf->Cell(25, 10, utf8_decode(MODENA . number_format($total, 2, '.', '')), 'LRTB', 0, 'C');
	...
	if ($datos_prestamo['prestamo_pagado'] < $datos_prestamo['prestamo_total']) {
		...
		$pdf->SetTextColor(97, 97, 97);
		$pdf->MultiCell(0, 9, utf8_decode("NOTA IMPORTANTE: \nEsta factura presenta un saldo pendiente de pago por la cantidad de " . MODENA . number_format(($datos_prestamo['prestamo_total'] - $datos_prestamo['prestamo_pagado']), 2, '.', '')), 0, 'J', false);
	}
	...
	<body>
		<div class="full-box container-404">
			<div>
				...
				<p class="lead text-center">No hemos enontrado el prestamo seleccionado</p>
			</div>
		</div>
